...
println(" T 1 ======"); whos(); println("========")


module Test

println(" TS 1 ======"); whos(); println("========")

	include("../src/SimpleMCMC.jl")

	DIFF_DELTA = 1e-8
	ERROR_THRESHOLD = 1e-3
println(" TS 2 ======"); whos(); println("========")
	...
	function test1(ex::Expr, x0::Union(Float64, Vector{Float64}, Matrix{Float64})) #  ex= :(2*x) ; x0 = 2.
		...
		assert(abs(gradn - grad0) < ERROR_THRESHOLD, "Gradient false for $ex at x=$x0, expected $(round(gradn,5)), got $(round(grad0,5))")
	end

	test1(e::Expr, v::Vector{Any}) = (for x in v; test1(e, x);end)
println(" TS 3 ======"); whos(); println("========")

end

################################

println(" T 2 ======"); whos(); println("========")
...
Test.test1(:(SimpleMCMC.logpdfUniform(-1, 1, x)), {-.1, 0., 1.})
Test.test1(:(SimpleMCMC.logpdfUniform(2, x, 1)), {0.5, 1., 10.})
Test.test1(:(SimpleMCMC.logpdfUniform(x, 1, 0.5)), {0.5, 1., 10.})
