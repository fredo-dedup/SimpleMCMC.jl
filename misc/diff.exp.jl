...
# func=:(logpdflogNormal(lmu, lsc, x::Array)) ; dv= :x ; diff= :( ( tmp2=lsc.*lsc ; res = ( (lmu - tmp2 - log(x)) ./ (tmp2.*x) ) .* ds ) )
...
## common operators
# TODO : remove [] within sums ? (now safe since sum redefinition ?)
...
@dfunc logpdfWeibull(sh::Real, sc, x)    sh  sum((1. - (x./sc).^sh) .* log(x./sc) + 1./sh * ds) .* ds
@dfunc logpdfWeibull(sh::Array, sc, x)   sh  ((1. - (x./sc).^sh) .* log(x./sc) + 1./sh * ds) .* ds
@dfunc logpdfWeibull(sh, sc::Real, x)    sc  sum(((x./sc).^sh - 1.) .* sh./sc * ds) .* ds
@dfunc logpdfWeibull(sh, sc::Array, x)   sc  (((x./sc).^sh - 1.) .* sh./sc * ds) .* ds
@dfunc logpdfWeibull(sh, sc, x::Real)    x   sum(((1. - (x./sc).^sh) .* sh - 1.) ./ x * ds) .* ds
@dfunc logpdfWeibull(sh, sc, x::Array)   x   (((1. - (x./sc).^sh) .* sh - 1.) ./ x * ds) .* ds
