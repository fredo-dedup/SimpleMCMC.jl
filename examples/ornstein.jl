...
simpleAGD(model, maxiter=1000, tau=20, mu=10., sigma=1.)

sigma=0.1
mu=10.
tau=20.
ftau, dummy = generateModelFunction(model, tau=0.05)
ftau2(x) = ftau([x])

plot(ftau2, 10, 40) | browse

simpleNM(model, tau=10, mu=10., sigma=1.)  
simpleNM(model, tau=20., mu=10)  
...
res = simpleHMC(model, steps=10000, burnin=1000, tau=1., mu=0., sigma=1.)

# run NUTS - HMC (1000 steps, 500 for burnin)
res = simpleNUTS(model, steps=10000, burnin=1000, tau=1., mu=0., sigma=1.)
